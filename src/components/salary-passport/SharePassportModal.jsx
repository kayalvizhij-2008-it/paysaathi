...
export default function SharePassportModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-[#19120A] border border-[#F5A623]/30 rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl relative overflow-hidden text-[#EEE0D2] text-center">
        ...
        <div className="mx-auto w-48 h-48 bg-white p-3 rounded-2xl shadow-xl flex items-center justify-center mb-4 mt-2">
          ...
          <div className="w-full h-full border-2 border-black p-2 flex flex-col justify-between">
            ...
            <div className="text-center font-mono text-[9px] font-bold text-black uppercase tracking-wider py-1">
              ...
              <span className="text-[#D97706]">{workerProfile.workerId}</span>
            </div>
        ...
        <p className="text-xs text-[#D7C3AE] mb-4">
          Scan to verify 36 months of authenticated wage history & 98/100 Trust Score.
        </p>

        {/* Copyable Link */}
        <div className="flex items-center gap-2 p-2 rounded-xl bg-[#251E16] border border-white/[0.1] text-xs font-mono text-[#D7C3AE] mb-5">
          <span className="truncate flex-1 text-left px-2">{shareUrl}</span>
          <button
            onClick={handleCopyLink}
            className="px-3 py-1.5 rounded-lg bg-[#F5A623] text-[#121212] font-bold shrink-0 hover:scale-105 active:scale-95 transition-all cursor-pointer"
          >
            Copy
          </button>
        </div>

        <div className="text-[11px] text-[#22C55E] flex items-center justify-center gap-1.5">
